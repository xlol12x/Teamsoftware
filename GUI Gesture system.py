...
#######################
#selecting controller tab rn will result in no output
#######################

#implement controller
#stop labels jumping around at bottom
#tidy up the code 
...
class App:
    def __init__(self, window, window_title):
        ...
        self.hands_title_label = tk.Label(left_canvas, text="", font=("Arial", 12, "bold underline"), bg="white")
        ...
        self.outputs_title_label = tk.Label(right_canvas, text="", font=("Arial", 12, "bold underline"), bg="white")
        self.outputs_title_label.pack(anchor="w", padx=(400,0))
        self.right_hand_output_label = tk.Label(right_canvas, text="", font=("Arial", 12), bg="white")
        self.right_hand_output_label.pack(anchor="w", padx=(400,0))
        self.left_hand_output_label = tk.Label(right_canvas, text="", font=("Arial", 12), bg="white")
        self.left_hand_output_label.pack(anchor="w", padx=(400,0))
        self.vertical_output_label = tk.Label(right_canvas, text="", font=("Arial", 12), bg="white")
        self.vertical_output_label.pack(anchor="w", padx=(400,0))
        self.horizontal_hand_output_label = tk.Label(right_canvas, text="", font=("Arial", 12), bg="white")
        self.horizontal_hand_output_label.pack(anchor="w", padx=(400,0))
    ...
    def update(self):
        if self.is_playing:
            ...
            if ret:
                ...
                if central_lm_x and central_lm_y:
                    ...
                    if active_tab_text == "Keyboard":
                        ...
                        if central_lm_x > square_size/2:
                            ...
                            self.horizontal_hand_output_label.configure(text = f"Horizontal Position Output: {self.right_key}")
                        elif central_lm_x < -square_size/2:
                            self.press_key(self.left_key)
                            self.horizontal_hand_output_label.configure(text = f"Horizontal Position Output: {self.left_key}")
                        else:
                        ...
                        if central_lm_y > square_size/2:
                            ...
                            self.vertical_output_label.configure(text = f"Vertical Position Output: {self.forward_key}")
                        elif central_lm_y < -square_size/2:
                            self.press_key(self.backward_key)
                            self.vertical_output_label.configure(text = f"Vertical Position Output: {self.backward_key}")
                        else:
                            self.vertical_output_label.configure(text = "Vertical Position Output:")
                #reset labels
                else:
                    self.right_hand_xcoord_label.config(text = f"Right Hand X Coordinates:")
                    self.right_hand_ycoord_label.config(text = f"Right Hand Y Coordinates:")
                    self.horizontal_hand_output_label.configure(text = "Horizontal Position Output:")
                    self.vertical_output_label.configure(text = "Vertical Position Output:")
                ...
                if active_tab_text == "Keyboard":
                    if right_gesture_name == 'Fist':
                        ...
                        self.right_hand_output_label.configure(text = f"Right Gesture Output: {self.right_fist_key}")
                    elif right_gesture_name == 'Call me':
                        self.press_key(self.right_call_key)
                        self.right_hand_output_label.configure(text = f"Right Gesture Output: {self.right_call_key}")
                    else:
                    ...
                    if left_gesture_name == 'Fist':
                        ...
                        self.left_hand_output_label.configure(text = f"Left Gesture Output: {self.left_fist_key}")
                    elif left_gesture_name == 'Call me':
                        self.press_key(self.left_call_key)
                        self.left_hand_output_label.configure(text = f"Left Gesture Output: {self.left_call_key}")
                    else:
            ...
            self.outputs_title_label.configure(text = "Current Output:")
            self.right_hand_output_label.configure(text = "Right Gesture Output:")
            self.left_hand_output_label.configure(text = "Left Gesture Output:")
            self.vertical_output_label.configure(text = "Vertical Position Output:")
            self.horizontal_hand_output_label.configure(text = "Horizontal Position Output:")
